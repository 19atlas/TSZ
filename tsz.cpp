...
void ArcExpandBuf(ArcCs *c)
{
  ...
  if (c->stack_ptr==c->stack_base && dst_ptr<dst_limit) {
    if (c->saved_basecode==0xFFFFFFFFl) {
      lastcode=ExtractWORDBitField(c->src_buf,c->src_pos,c->next_bits_in_use);
    ...
    while (dst_ptr<dst_limit &&
      ...
      basecode=ExtractWORDBitField(c->src_buf,c->src_pos,c->next_bits_in_use);
      c->src_pos=c->src_pos+c->next_bits_in_use;
    ...
    if (out_buf=ExpandBuf(in_buf)) {
      if (cvt_ascii) {
	      ...
	      for (i=0;i<out_size;i++){
	        if (out_buf[i]==31){out_buf[j++]=32;}
	        else if (out_buf[i]!=5){out_buf[j++]=out_buf[i];}
        }
	      out_size=j;
      }
...
int main(int argc, char* argv[]){
  ...
  if (okay){
    ...
    puts("TSZ [-ascii] filename\r\n\r\n"
	 "TSZ expands a single SparrowOS file.\r\n"
	 "The -ascii flag will convert nonstandard SparrowOS\r\n"
